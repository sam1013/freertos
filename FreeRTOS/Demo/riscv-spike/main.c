...
#define mainCHECK_TIMER_PERIOD_MS			( 1000UL / portTICK_PERIOD_MS )
...
static void privCreateTasks( void );
/*-----------------------------------------------------------*/

void privCreateTasks( void )
{
	...
	configASSERT(pdPASS == xTaskCreate( vTaskA, "TaskA", configMINIMAL_STACK_SIZE, NULL, bktPRIMARY_PRIORITY, NULL ));
	configASSERT(pdPASS == xTaskCreate( vTaskB, "TaskB", configMINIMAL_STACK_SIZE, NULL, bktPRIMARY_PRIORITY, &xSecondary ));
}
/*-----------------------------------------------------------*/


static void assertPriv(BaseType_t expectedPriv) {
  BaseType_t xRunningPrivileged = xPortRaisePrivilege();
  vPortResetPrivilege( xRunningPrivileged );
  configASSERT(xRunningPrivileged == expectedPriv);
}
...
{
	printf("Hello world!\n");
	for (size_t i = 0; i < 500000; i++) //to allow openocd halt the CPU before spike aborts
	  ;
